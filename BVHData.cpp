...
{ // ReadFileBVH()
    ...
    while (std::getline(inFile, line)) { // more lines in the file
        // not an empty line
        if (line.size() != 0) { // non-empty line
            ...
            if (tokens[0] == "HIERARCHY") { // hierarchy
            ...
            else if (tokens[0] == "MOTION") { // motion
                ReadMotion(inFile);
                break;
            }      // motion
            else { // otherwise
...
{ // StringSplit()
    ...
    while (!iss.eof()) { // more tokens to come
        // create a token
        std::string token;
        // read it in
        iss >> token;
...
void BVHData::ReadHierarchy(std::ifstream &inFile,
...
{ // ReadHierarchy()
    ...
    if (line[0] == "{") { // group of children
        // ignore the rest of the line and read in a new one
        NewLine(inFile, line);
        while (line[0] != "}") { // until we hit the close of the group
...
{ // ReadMotion()
    ...
    while (std::getline(inFile, line)) { // more data
        // ignore empty lines
        if (line.size() != 0) { // non-empty line
            ...
            for (size_t i = 0; i < tokens.size(); i++) { // per token
                if (!isNumeric(tokens[i])) {             // if it's not numeric
...
{ // Cylinder()
    ...
    for (int i = 0; i < slices; i++) { // per slice
        ...
        Homogeneous4 c_edge1 = viewMatrix
                               * Homogeneous4(radius * cos(theta), radius * sin(theta), Length, 1);
        Homogeneous4 c_edge2 = viewMatrix
                               * Homogeneous4(radius * cos(nextTheta),
                                              radius * sin(nextTheta),
                                              Length,
                                              1);
        // and two points on the bottom circle
        Homogeneous4 c_edge3
            = viewMatrix * Homogeneous4(radius * cos(nextTheta), radius * sin(nextTheta), 0, 1);
        Homogeneous4 c_edge4 = viewMatrix
                               * Homogeneous4(radius * cos(theta), radius * sin(theta), 0, 1);
        ...
        // one for the middle
        Cartesian3 normal_edge = viewMatrix * Cartesian3(cos(midTheta), sin(midTheta), 0.0)
                                 - viewMatrix * Cartesian3(0.0, 0.0, 0.0);
...
// load all rotation and translation data into this class
void BVHData::loadAllData(std::vector<std::vector<Cartesian3>> &rotations,
                          std::vector<Cartesian3> &translations,
                          std::vector<std::vector<float>> &frames)
{ // loadAllData()
    // store all rotations
    for (size_t i = 0; i < frames.size(); i++) { // per frame
    ...
    for (size_t i = 0; i < this->all_joints.size(); i++) { // per joint
...
{                                                           // loadRotationData()
    for (size_t j = 0, j_c = 0; j < frames.size(); j_c++) { // per frame
        ...
        {                                                                        // per channel
            ...
            if (this->all_joints[j_c]->joint_channel[k].substr(1)
                == "rotation") { // rotation channel
